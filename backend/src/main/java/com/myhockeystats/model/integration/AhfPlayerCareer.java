...
/**
 * AhfPlayerCareer entity - represents a single player career record in AHF.
 * Maps to ahf_player_career table created by scripts/thf-js/scrape_rosters.js
 * One row per (source_player_id, season_label).
 */
@Entity
@Table(name = "ahf_player_career", indexes = {
    @Index(name = "idx_ahf_career_player_season", columnList = "source_player_id, season_label"),
    @Index(name = "idx_ahf_career_normalized_name", columnList = "player_name")  // Functional index on LOWER(TRIM(player_name))
})
public class AhfPlayerCareer {
