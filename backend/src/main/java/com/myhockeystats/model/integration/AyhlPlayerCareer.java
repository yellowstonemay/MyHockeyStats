...
/**
 * AyhlPlayerCareer entity - represents a single player career record in AYHL.
 * Maps to ayhl_player_career table created by scripts/ayhl/daily_update.py
 * One row per (source_player_id, season_label).
 */
@Entity
@Table(name = "ayhl_player_career", indexes = {
    @Index(name = "idx_ayhl_career_player_season", columnList = "source_player_id, season_label"),
    @Index(name = "idx_ayhl_career_normalized_name", columnList = "player_name")  // Functional index on LOWER(TRIM(player_name))
})
public class AyhlPlayerCareer {
